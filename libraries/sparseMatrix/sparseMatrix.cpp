...
{
    SparseMatrixOutline emptyOutline(1);
    InitFromOutline(&emptyOutline);
}
...
void SparseMatrix::GetDiagonal(double * diagonal)
{
	if (diagonalIndices != NULL)
	{
		assert(!diagonalIndicesOutOfDate && "diagonal indices are out of date, please rebuild");
		for(int i=0; i<numRows; i++)
...
double SparseMatrix::SkewSymmetricCheck()
{
	...
	{    
		...
		{      
			...
			int tindex = transposedIndices[i][j];
...
{
	assert(!transposedIndicesOutOfDate && "transposed indices out of date");
	for(int i=0; i<numRows; i++)
	{
		for(int j=0; j<rowLength[i]; j++)
		{
			...
			columnEntries[i][j] = columnEntries[jAbs][transposedIndices[i][j]];
		}
	}
}
...
// open a new entry in the sparse matrix
void SparseMatrix::AddSparseEntry(int row, int denseJ)
{
	assert(row<=GetNumRows() && "row out of bounds");
	assert(denseJ<=GetNumColumns() && "column out of bounds");
...
void SparseMatrix::AddSparseBlock3x3Entry(int startRow, int startDenseJ)
{
...
{
	assert(!superMatricesOutOfDate && "supermatrix indices are out of date, please rebuild");
	for(int i=0; i<numRows; i++)
	{
...
int SparseMatrix::BuildSubMatrixIndices(SparseMatrix & submatrix, int subMatrixID)
{
    if (subMatrixID==-1)
    {
...
{
	assert(subMatrixID < numSubMatrixIDs && "Warning: attempted to free submatrix index that does not exist.");
    ...
    {
        printf("Warning: attempt to FreeSubMatrixIndices on already-freed submatrix %i", subMatrixID);
    }
